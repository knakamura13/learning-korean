<script lang="ts">
	...
	import { plateTextKind, plateTextMeta, type PlateTextId } from '$lib/domain/plateText';

	let { plateId }: { plateId: PlateTextId } = $props();

	const meta = $derived(plateTextMeta(plateId));
	const kind = $derived(plateTextKind(plateId));
...
<article class="plate-text" data-kind={kind}>
	<header class="head">
		{#if meta.labNumber !== null}
			<p class="eyebrow">Plate {String(meta.labNumber).padStart(2, '0')}</p>
		{:else}
			<p class="eyebrow">Apparatus</p>
		{/if}
		<h1>{meta.title}</h1>
		<p class="lede">{meta.caption}</p>
	</header>
				...
				{#each REPRESENTATIVE as r (r)}
					<div class="row">
						...
						<span class="vals hg" lang="ko">
							{BATCHIM_LETTERS.filter((f) => batchimSound(f) === r).join('  ')}
						</span>
					</div>
				{/each}
			</div>
		</section>
	{:else if plateId === '0005'}
	...
	{:else if plateId === '0006'}
		<section id="sound-changes" aria-labelledby="sec-sound-changes-heading">
			...
			<p class="lede tiny muted">
				Korean spelling preserves what a word <em>is</em>; these rules are how it sounds.
			</p>
			<div class="rows card">
	...
	{:else if plateId === 'dictionary-order'}
	...
	{:else if plateId === 'sources'}
		<section id="sources" aria-labelledby="sec-sources-heading">
			...
			<ul class="src">
				...
				<li>
					<a href="https://www.koreascience.kr/article/JAKO202225852210743.page"
						>“Hunminjeongeum Phonetics (II)”</a
					>
				...
				<li>
					...
					— free, with native audio for every letter.
				</li>
			</ul>
		</section>
	{/if}
