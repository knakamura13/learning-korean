...
import { lockedLabPopoverCopy, placeClickPopover } from './lockedLab';
...
describe('placeClickPopover', () => {
	...
	it('flips up and toward the start when the click is in the end corner', () => {
		...
		expect(placed.left + 280).toBeLessThanOrEqual(800 - 8);
	...
	it('never leaves the viewport, even on a small phone', () => {
		...
		expect(placed.left + 300).toBeLessThanOrEqual(390 - 8);
		expect(placed.top + 240).toBeLessThanOrEqual(720 - 8);
	});
});
