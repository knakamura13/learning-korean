...
export const FONT_PRELOAD_PLACEHOLDER = '%%DESIGN_FONT_PRELOADS%%';

export function fontPreloadLinks(system: DesignSystem): string {
	return system.fonts
		.map(
			(face) =>
				`<link rel="preload" href="%sveltekit.assets%/fonts/${face.file}" as="font" type="font/woff2" crossorigin="anonymous" />`
		)
		.join('\n\t\t');
}
...
export function applyDesignSystem(html: string, system: DesignSystem): string {
	return html
		...
		.replaceAll(FONT_PRELOAD_PLACEHOLDER, fontPreloadLinks(system));
}
