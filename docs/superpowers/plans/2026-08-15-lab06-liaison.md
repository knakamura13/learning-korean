...
- `applyLiaison` is Articles 13–14 only: no palatalization, no Article 15 representative-sound compounds, no general tensification. Moved cluster-ㅅ becomes ㅆ. ㅇ-batchim and ㅎ-batchim are no-ops.
...
- Modify: `app/src/lib/domain/hangul.ts` — liaison + romanization
...
- Modify: `app/src/routes/review/reviewAnswerField.test.ts` — placeholder still labeled
...
```bash
git add app/src/lib/domain/hangul.ts app/src/lib/domain/hangul.test.ts
git commit -m "feat(hangul): romanize spoken syllable cuts"
```
...
6. Replace `checkAnswer` / add `normalizePron`:
...
**Files:**
- Create: `app/src/lib/content/lab06.ts` (full contents below — do not summarize)
...
2. Input `placeholder={pronCard ? 'han-gu-geo or 한구거' : 'type the romanization'}`
3. Empty hint: `pronCard ? 'Type hyphenated cuts, or Hangul, then Check.' : 'Type a romanization, then Check.'`
...
**Files:**
- Modify: `NOTES.md` — in “Backlog of lesson ideas”, mark Lab 06 liaison as built; next item is tensification + nasalization. Delete or clearly mark the stale duplicate numbered list that still puts liaison after unbuilt clusters.
...
|---|---|
| `applyLiaison` Articles 13–14, ㅇ/ㅎ no-op, ㅅ→ㅆ, no palatalization | 1 |
...
Tensification lab, nasalization, audio clips for these words, Article 15, handwriting, a `pron` jamo picker.
